...
def main():
    ...
    # 6) Összefoglalók (overall + csoportbontások)
    def summ(s: pd.Series) -> pd.Series:
        s = s.dropna()
        return pd.Series({
            "N": len(s),
            "mean": s.mean(),
    ...
    if not trades_df.empty:
        overall = summ(trades_df["pnl_pct"])
        overall_df = pd.DataFrame(overall, columns=["pnl_pct"])
    else:
        overall_df = pd.DataFrame(columns=["pnl_pct"])

    overall_path = outdir / "schedule13_stop_summary.csv"
    overall_df.to_csv(overall_path)

    # --- Helper: összeállít egy oszlopos (pnl_pct) táblát több csoportra ---
    def grouped_summary(df: pd.DataFrame, by_cols: list[str]) -> pd.DataFrame:
        """
        Visszaad egy DataFrame-et, ahol a sorok a statisztikai mutatók (N, mean, …),
        az oszlopok pedig a csoportok (MultiIndex is lehet, pl. (form_type, cap_bucket)).
        """
        if df.empty:
            return pd.DataFrame()
        tables = {}
        # dropna=False → a NaN kategóriák is megjelennek külön csoportként
        for key, sub in df.groupby(by_cols, dropna=False):
            # kulcs normalizálása oszlopnévnek
            if not isinstance(key, tuple):
                col_name = (key,)
            else:
                col_name = key
            tables[col_name] = summ(sub["pnl_pct"])
        # többdimenziós (MultiIndex) oszlopok jönnek létre, ha több a csoport
        out = pd.concat(tables, axis=1)
        # rendezés, hogy olvashatóbb legyen
        out = out.sort_index(axis=1)
        return out

    # --- form_type szerinti összefoglaló ---
    if "form_type" in trades_df.columns:
        sum_by_form = grouped_summary(trades_df, ["form_type"])
    else:
        sum_by_form = pd.DataFrame()
    (outdir / "schedule13_stop_summary_by_form.csv").write_text(
        sum_by_form.to_csv() if not sum_by_form.empty else ""
    )

    # --- cap_bucket szerinti összefoglaló ---
    if "cap_bucket" in trades_df.columns:
        sum_by_cap = grouped_summary(trades_df, ["cap_bucket"])
    else:
        sum_by_cap = pd.DataFrame()
    (outdir / "schedule13_stop_summary_by_cap.csv").write_text(
        sum_by_cap.to_csv() if not sum_by_cap.empty else ""
    )

    # --- form_type × cap_bucket kereszttábla ---
    if {"form_type", "cap_bucket"}.issubset(trades_df.columns):
        sum_by_form_cap = grouped_summary(trades_df, ["form_type", "cap_bucket"])
    else:
        sum_by_form_cap = pd.DataFrame()
    (outdir / "schedule13_stop_summary_by_form_cap.csv").write_text(
        sum_by_form_cap.to_csv() if not sum_by_form_cap.empty else ""
    )
    ...
    if not trades_df.empty:
        ...
        print(f"Eredmények (trade szint): {trades_path}")
        print(f"Összefoglaló (overall):   {overall_path}")
        print(f"Összefoglaló (form):      {outdir / 'schedule13_stop_summary_by_form.csv'}")
        print(f"Összefoglaló (cap):       {outdir / 'schedule13_stop_summary_by_cap.csv'}")
        print(f"Összefoglaló (form×cap):  {outdir / 'schedule13_stop_summary_by_form_cap.csv'}")
    else:
        print("Nem született érvényes trade – futtasd nagyobb post window-val (pl. --post-days 240), vagy ellenőrizd a TP/stop beállításokat.")
